	...
	\begin{tabularx}{\linewidth}{|X|X|X|}
		...
		<? for (const [rowIndex, row] of data.entries()) { ?>
			...
			<? if (row.minutesBoiled !== data[rowIndex - 1]?.minutesBoiled) { ?>
				<?
					const end = data.findIndex(dataRow => dataRow.minutesBoiled !== row.minutesBoiled, rowIndex);
					const trialIndexes = R.range(rowIndex, end === -1 ? data.length : end);
					const trialMoles = trialIndexes.map(trialIndex => calculateMolesOfChlorine(trialIndex));
					...
					const uncertaintyPercentOfAverage = sumOfUncertaintyOfAverage / R.sum(trialMoles)
					const uncertaintyOfAverage = averageMolesOfChlorine * uncertaintyPercentOfAverage;
				?>
				\multirow{<?= R.count(r => r.minutesBoiled === row.minutesBoiled, data) ?>}{*}{<?= averageMolesOfChlorine ?>}
				\\\cline{1-2}
